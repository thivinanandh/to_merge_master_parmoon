...
TParFEMapper3D::TParFEMapper3D(int N_dim, TFESpace3D *fespace, int *rowptr, int *kcol)
{
  ...
  if(smoother==6 || smoother==20 || smoother==21 || smoother==22 || smoother==23 ){
...
void TParFEMapper3D::colorPDOF(int N_U, int N_Cells, TCollection *Coll, char *DofmarkerU){
...
//     int **UDOFs= new int*[N_PDOF];
    
    int N_PDOF = FESpace->GetN_DegreesOfFreedom();
    
    vector<vector<int>> UDOFs(N_PDOF);
...
//     cout<<"neha:MaxN_LocalU"<<MaxN_LocalU<<endl;
    ...
    vector<int> nonHaloDOF(N_OwnDof); 
    int r=0;
...
//         if(max_NU < UDOFs[j].size()){
//                 max_NU= UDOFs[j].size();
//             }
//         
//         cout<<UDOFs[j].size()<<endl;

    }
    
    cout<<"neha:max_NU"<<max_NU<<endl;
...
#pragma omp parallel for default(shared) num_threads(2) private(ii) collapse(2)
  for( ii=0; ii<N_OwnDof; ii++){
      
//       if(DofMarker[ii] == 'h' || DofMarker[ii] == 'H'  ||  DofMarker[ii] == 's')
//             continue;
      
      for(jj=0; jj<N_OwnDof; jj++){
...
//             allocatedColor[0]=0;
//             
//             PDOFs = PGlobalNumbers+PBeginIndex[0];
//     
//             
//         //     cout<<"N_P:"<<N_P<<endl;
//             for(int kk=0; kk<N_P; kk++){
//                 PDOF = PDOFs[kk];
//                 allocatedPColor[PDOF]=l;
//                 l++;
//             }
//         }
//         avail[allocatedColor[0]]=true;
// #endif
  ...
  t1=MPI_Wtime();
    ...
    for(int ii=0;ii<N_PDOF;ii++)
    {
        if(DofMarker[ii] == 'h' || DofMarker[ii] == 'H'  ||  DofMarker[ii] == 's')
            continue;

    
        for(int jj=0;jj<adjlist[ii].size();jj++)
        {
//             if(rank==1)
//             cout<<"ii:"<<ii<<"jj"<<adjlist[ii][jj]<<endl;
            if(allocatedPColor[adjlist[ii][jj]] != -1){
                
                avail[allocatedPColor[adjlist[ii][jj]]]= true;
            }
            
        }
        
        int cr; 
        for (cr = 0; cr < N_PDOF; cr++) {
            if (avail[cr] == false) 
                break;
        }
        
        allocatedPColor[ii] = cr;
//         if(rank==1)
//             cout<<"ii:"<<ii<<"color:"<<allocatedColor[ii]<<endl;
//         l=0;
//         for(int kk=0; kk<N_P; kk++){
//             PDOF = PDOFs[kk];
//             
//             if(DofMarker[PDOF] == 'h' || DofMarker[PDOF] == 'H'  ||  DofMarker[PDOF] == 's')
//                 continue;
//             allocatedPColor[PDOF]=cr*N_P+l;
//             l++;
//         }
        ...
        for(int jj=0;jj<adjlist[ii].size();jj++){
            ...
            if(allocatedPColor[adjlist[ii][jj]] != -1){
                avail[allocatedPColor[adjlist[ii][jj]]]= false;
            }
            
        }
        
        if(numColors < allocatedPColor[ii])
        numColors = allocatedPColor[ii];
    }
//     }
...
//                 if(rank==0)
//                 cout<<"PDOFReorder:"<<PDOFReorder[k]<<"i:"<<i<<endl;
//                 if(rank==1)
//                 cout<<"PDOFReorder:"<<PDOFReorder[k]<<"i:"<<i<<endl;
                k++;
                allocatedPColor[j] = -1;
...
// void TParFEMapper3D::colorPDOF(int N_U, int N_Cells, TCollection *Coll){
//     
//     // #ifdef __3D__
//   const int MaxN_LocalU =  4*125;//4*MaxN_BaseFunctions3D;
//   TFE3D *UEle, *PEle;
...
// //   }
//   
//     vector<vector<int>> adjlist(N_Cells);
//     vector<bool> isHalo(N_Cells,false);
//     int ii,jj;
//     
//     for( ii=0; ii<N_Cells; ii++){
...
//     
//     
// // #pragma omp parallel for default(shared) num_threads(4) private(ii) collapse(2)
//   for( ii=0; ii<N_Cells; ii++){
//       
//       for(jj=0; jj<N_Cells; jj++){
//                 
//                     
//             if(match(UDOFs[ii], UDOFs[jj], N_U)){
//                 
//                 adjlist[ii].emplace_back(jj);
//         //         if(rank==0){
//         //         cout<<"ii"<<ii<<endl;
//         //         cout<<"jj"<<jj<<endl;
//         //         }
...
// //             if(rank==1)
// //             cout<<"ii:"<<ii<<"jj"<<adjlist[ii][jj]<<endl;
//             if(allocatedColor[adjlist[ii][jj]] != -1){
//                 
//                 avail[allocatedColor[adjlist[ii][jj]]]= true;
//             }
//             
//         }
//         
//         int cr; 
//         for (cr = 0; cr < N_Cells; cr++) {
//             if (avail[cr] == false) 
//                 break;
//         }
//         
...
//             
//             if(allocatedColor[adjlist[ii][jj]] != -1){
//                 avail[allocatedColor[adjlist[ii][jj]]]= false;
//             }
//             
//         }
//         
...
bool TParFEMapper3D::match(vector<int> &list1, vector<int> &list2){
...
bool TParFEMapper3D::match(int *list1, int *list2, int N_U){
...
void TParFEMapper3D::colorCell(int N_U, int N_Cells, int N_OwnCells, TCollection *Coll){
...
// //                 cout<<"thrdid:"<<omp_get_thread_num()<<endl;
// //                 cout<<"diff:"<<ptrCellColors[i+1]-ptrCellColors[i]<<endl;
// //                 cout<<"thrd id:"<<omp_get_thread_num()<<endl;
...
//     #endif
        ...
        {
            // if(rank==0)
            // cout<<"adj:"<<ii<<":"<<jj<<":"<<adjlist[ii][jj]<<endl;
...
//   }
  ...
  t1=MPI_Wtime();
    ...
    for(i=0;i<ndof;i++)
    {
        ...
        for(int j=RowPtr[k];j<RowPtr[k+1];j++)
        {
            if(KCol[j] >= k || DofMarker[KCol[j]] != type)	continue;
            //if(DofMarker[KCol[j]] != type)	continue;
            
            int t = NewGN[KCol[j]];		    //locate the pos of the dof in myreorder to identify its color in allocatedColor
            
            if(allocatedColor[t] != -1){
                avail[allocatedColor[t]]= true;
            }
...
//     }
    ...
    t2=MPI_Wtime();
    printf("total time taken for coloring %c type dof = %lf\n",type,t2-t1);  
    //colors were numbered from 0, hece total will be 1 more
    numColors++;
    
  ptrColors = new int[numColors+1];
  int *temp_Reorder = new int[ndof];
  temp = 0; 
  k = 0;
  //arrange the dofs, such that same color dofs are together
  for(i=0;i<numColors;i++)
