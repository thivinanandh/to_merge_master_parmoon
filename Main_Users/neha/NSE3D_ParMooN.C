...
//     cout<<"neha1:"<<TDatabase::ParamDB->SC_SOLVER_SADDLE<<endl;
  Database->CheckParameterConsistencyNSE();
//   cout<<"neha2:"<<TDatabase::ParamDB->SC_SOLVER_SADDLE<<endl;
...
//         SystemMatrix->Assemble();
    ...
    double start_assembling=0;
    double assembling_time=0;
    double start_solving=0;
  ...
  if(profiling){
#ifdef _MPI
    assembling_time += MPI_Wtime()-start_assembling;
#else
    assembling_time += GetTime()-start_assembling;
#endif
  }
    ...
    for(j=1;j<=Max_It;j++)
     {
                 ...
                 if(profiling){
#ifdef _MPI
      start_solving = MPI_Wtime();
#else
      start_solving = GetTime();
#endif
    } 
      ...
      SystemMatrix->Solve(sol, rhs);
              ...
              if(profiling){
#ifdef _MPI
      solving_time += MPI_Wtime()-start_solving;
#else
      solving_time += GetTime()-start_solving;
#endif
    }
...
//====================================================================== 
  ...
  if(profiling){
    ...
    OutPut("Total Cells :: "<<N_Cells<<"     Total_dof_U :: "<<N_U<<"      Total_dof_P :: "<<N_P<<"      Total_dof :: "<<N_U+N_P<<endl<<endl);
    OutPut("----------------------------------------------------------------------------------------------------------------------"<<endl);
...
//     OutPut( "Total time taken for solving : " << (total_solve) << "("<<100*(total_solve)/(stop_time-start_time)<<"%)"<<endl);
    OutPut( "Total time taken for assembling : " << (assembling_time) << "("<<100*(assembling_time)/(stop_time-start_time)<<"%)"<<endl);
        
    OutPut( "Total time taken for solving : " << (solving_time) << "("<<100*(solving_time)/(stop_time-start_time)<<"%)"<<endl);
    
    OutPut( "Total time taken for vanka assembling : " << (timeVankaAssemble) << "("<<100*(timeVankaAssemble)/(stop_time-start_time)<<"%)"<<endl);
    
    OutPut( "Total time taken for vanka solving : " << (timeVankaSolve) << "("<<100*(timeVankaSolve)/(stop_time-start_time)<<"%)"<<endl);
    
    OutPut( "Total time taken for data transfer : " << (data_transfer_time/1000) << "("<<100*(data_transfer_time/1000)/(stop_time-start_time)<<"%)"<<endl);
    
    OutPut( "Total time taken for kernel execution : " << (kernel_time/1000) << "("<<100*(kernel_time/1000)/(stop_time-start_time)<<"%)"<<endl);
