...
void Exact(double x, double y, double *values)
{
  ...
  values[0] = exp(-0.2*t)*(sin(Pi*x));
  values[1] = exp(-0.2*t)*Pi*cos(Pi*x);
  values[2] = 0;
  values[3] = -1*exp(-0.2*t)*Pi*Pi*sin(Pi*x);
}
...
void BoundValue(int BdComp, double Param, double &value)
{
  ...
  if(BdComp==0)
    value = exp(-0.2*t)*(sin(Pi*Param));
  else if (BdComp == 1)
      value = 0;
  else if (BdComp == 2)
      value = exp(-0.2*t)*(sin(Pi*(1-Param)));
  else
    value = 0;
}
        ...
        double **parameters, double **coeffs)
{
  ...
  for(i=0;i<n_points;i++)
  {
    ...
    coeff[4] = -0.2*exp(-0.2*t)*(sin(Pi*x))
	+ eps * exp(-0.2*t)*Pi*Pi*sin(Pi*x)
       + b1 * exp(-0.2*t)*Pi*cos(Pi*x)
       + b2 * 0
	+ c *  exp(-0.2*t)*(sin(Pi*x));
  }
}
